...
var latLngs = [];///construir El camino
var path = null;
var followRoad = "false";
...
var adUnit;
/*Se situa el mapa en El Salvador*/
latitude = '13.701400649368265';
longitude = '-89.22391891479492';
function initMap(){
	var mapProp = {
    ...
    if (markers && markers.length > 1) {
        ...
        directionService.route(request, function (response, status) {
            if (status == google.maps.DirectionsStatus.OK) {
                ...
                alert("No es una ruta especifica");
            }
        });
    } else {
        latLngs.push(markers[markers.length - 1].getPosition());
...
function clearOverlays() {
    var cnf = confirm("Está a punto de borrar todos los puntos de la ruta.\n ¿Desea hacer eso?");
    if (cnf == true) {
        if (markers) {
            for (i in markers) {
...
function saveMapToServer(){
   ...
   ;
    alert("Se guardo en el servidor");
     ...
     $.ajax({
        ...
        success:function(data){
            ...
            if(data > -1 ){
                ...
                $("#route_id_done").html("<a href='viewMap.php?route="+data+"'> Mirar Ruta </a>" );
            }else{
